...
import { gaokaoMeta, gaokaoSubjects, gaokaoTracks } from "../src/data/gaokao.js";
...
const labels = {
  "zh-Hant": {
    ...
    duration: "考試時間 90 分鐘",
    total: "滿分 150 分",
    instruction: "所有題目均為單選題。請把答案填在另紙；列印前確認組別與頁碼。",
    print: "列印／另存 PDF",
    answer: "答案",
    explanation: "解析",
    marks: "分",
    generated: "本檔可完全離線閱讀；題號與線上模擬一致。",
  },
  ja: {
    title: "幻想郷統一高等学力試験",
    paper: "問題冊子",
    ...
    duration: "試験時間 90分",
    total: "満点 150点",
    instruction: "全問単一選択。解答は別紙へ記入し、印刷前に区分と頁を確認すること。",
    print: "印刷／PDF保存",
    answer: "解答",
    explanation: "解説",
    marks: "点",
    generated: "完全オフラインで閲覧可能。問題番号はオンライン模試と共通です。",
  },
  en: {
    ...
    duration: "Time allowed: 90 minutes",
    total: "Maximum: 150 marks",
    instruction: "All items are single-choice. Record answers separately; confirm the track and page order before printing.",
    print: "Print / Save as PDF",
    answer: "Answer",
    explanation: "Explanation",
    marks: "marks",
    generated: "This file works fully offline. Question IDs match the online simulation.",
  },
};
...
function render(trackId, locale, answerKey) {
...
<head>
  ...
  <title>${c.title} · ${track.name[locale]} · ${answerKey ? c.answers : c.paper}</title>
  <style>
    ...
    *{box-sizing:border-box}body{margin:0}.toolbar{position:sticky;top:0;z-index:2;display:flex;padding:12px 5vw;align-items:center;justify-content:space-between;color:#fff;background:#172536}
    ...
    .cover{min-height:740px;padding:80px 45px;border:4px double #18222e;text-align:center;break-after:page}.cover>p{letter-spacing:.2em}.cover h1{margin:90px 0 15px;font-family:serif;font-size:44px}.cover h2{color:#8e2f37;font-family:serif;font-size:26px}
    .cover dl{margin:70px auto 30px;max-width:560px;text-align:left}.cover dl div{display:grid;padding:12px;grid-template-columns:150px 1fr;border-bottom:1px solid #18222e}.cover dd{margin:0}
    ...
    article{padding:28px 0;border-bottom:1px solid #ddd;break-inside:avoid}article h3{display:grid;margin:0;grid-template-columns:48px 1fr auto;gap:8px;font-family:serif;font-size:16px;line-height:1.65}article h3>span{color:#8e2f37;font:800 11px system-ui}article h3 small{font:700 10px system-ui;color:#8e2f37}article ol{margin:15px 0 0 70px;padding-left:25px}article li{padding:5px;line-height:1.55}
    ...
    @media(max-width:650px){main{padding:30px 20px}.cover{min-height:0;padding:45px 18px}.cover h1{margin-top:55px;font-size:32px}article h3{grid-template-columns:42px 1fr}article h3 small{grid-column:2}article ol,.key{margin-left:20px}}
...
<body>
  <div class="toolbar"><span>${gaokaoMeta.edition} · ${track.name[locale]} · ${answerKey ? c.answers : c.paper}</span><button onclick="window.print()">${c.print}</button></div>
  <main>
    <header class="cover">
      <p>${gaokaoMeta.edition} · ${gaokaoMeta.year}</p>
      <h1>${c.title}</h1>
      <h2>${track.name[locale]} · ${answerKey ? c.answers : c.paper}</h2>
      <dl><div><dt>${c.candidate}</dt><dd>____________________________</dd></div><div><dt>${c.number}</dt><dd>____________________________</dd></div><div><dt>${c.duration}</dt><dd>${c.total}</dd></div></dl>
...
for (const locale of locales) {
  for (const trackId of Object.keys(gaokaoTracks)) {
    for (const answerKey of [false, true]) {
      ...
      const filename = `${gaokaoMeta.edition.toLowerCase()}-${locale}-${trackId}-${suffix}.html`;
      await writeFile(path.join(output, filename), render(trackId, locale, answerKey), "utf8");
      count += 1;
    }
  }
}
